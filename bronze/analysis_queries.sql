...
category,
purchase_count,
ROW_NUMBER() OVER(PARTITION BY age_group ORDER BY purchase_count DESC) AS rn
FROM category_count
...
SELECT 
monthly_sales AS months,
yearly_sales AS years,
total_amount,
LAG(total_amount) OVER (ORDER BY yearly_sales, monthly_sales) AS prev_amount,
((total_amount - LAG(total_amount) OVER (ORDER BY yearly_sales, monthly_sales)) 
/ NULLIF(LAG(total_amount) OVER (ORDER BY yearly_sales, monthly_sales),0)) * 100 
